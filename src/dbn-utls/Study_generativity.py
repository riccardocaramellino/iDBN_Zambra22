  ...
  if Load_DBN_yn == 0:
    ...
    for run in range(RUNS):
        ...
        if not('CelebA' in DATASET_ID):
          ...
          dbn.Num_classes = 40
          compute_inverseW_for_lblBiasing_ZAMBRA(dbn,train_dataset, L = train_dataset['labels'])
        name = name +'_'+str(dbn.Num_classes)+'classes_nEp'+str(EPOCHS)+'_nL'+str(len(dbn.rbm_layers))+'_lastL'+str(dbn.top_layer_size)
    ...
    dbn = torch.load(os.path.join(Zambra_folder_drive, 'dbn_iterative_normal_'+DATASET_ID+'_'+str(Num_classes)+'classes_nEp'+str(EPOCHS)+'_nL'+str(nL)+'_lastL'+last_layer_sz+'.pkl'))
...
def Plot_example_generated(input_dict, model,row_step = 10, dS=20, custom_steps = True, Show_classification = False, not_random_idxs = True):
    side_img = int(np.sqrt(model.rbm_layers[0].Nin))
    ...
    for sample_idx in random_numbers: #per ogni sample selezionato
        ...
        reconstructed_img = reconstructed_img.view((side_img,side_img)).cpu() #ridimensiono l'immagine e muovo su CPU
        ...
        for idx,step in enumerate(steps): # idx = riga dove plotterò, step è il recostruction step che ci plotto
            ...
            reconstructed_img = reconstructed_img.view((side_img,side_img)).cpu()
